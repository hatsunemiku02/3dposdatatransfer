...
class MyModel(object):
  ...
  def __init__(self,
    ...
    with vs.variable_scope( "linear_model" ):
      ...
      if remove_max_normal:
        w1 = tf.assign(w1,tf.clip_by_norm(w1,1))
      #w1 = tf.clip_by_norm(w1,1) if max_norm else w1
      y3 = tf.matmul( enc_in, w1 ) + b1
      if batch_norm:
        y3 = tf.layers.batch_normalization(y3,training=False,trainable=False, name="batch_normalization")
      y3 = tf.nn.relu( y3 )
      #y3 = tf.nn.dropout( y3, self.dropout_keep_prob )

      # === Create multiple bi-linear layers ===
      for idx in range( num_layers ):
        y3 = self.two_linear_my( y3, linear_size, residual, max_norm, batch_norm, dtype, idx,remove_max_normal )
      ...
      if remove_max_normal:
        w4 = tf.assign(w4,tf.clip_by_norm(w4,1))
  ...
  def two_linear_my( self, xin, linear_size, residual, max_norm, batch_norm, dtype, idx ,remove_max_normal):
    """
    ...
    with vs.variable_scope( "two_linear_"+str(idx) ) as scope:
      ...
      if remove_max_normal:
        w2 = tf.assign(w2,tf.clip_by_norm(w2,1))
      #w2 = tf.clip_by_norm(w2,1) if max_norm else w2
      y = tf.matmul(xin, w2) + b2
      if  batch_norm:
        y = tf.layers.batch_normalization(y,training=False,trainable=False,name="batch_normalization1"+str(idx))
      ...
      if remove_max_normal:
       w3 = tf.assign(w3,tf.clip_by_norm(w3,1))
      #w3 = tf.clip_by_norm(w3,1) if max_norm else w3
      y = tf.matmul(y, w3) + b3

      if  batch_norm:
        y = tf.layers.batch_normalization(y,training=False,trainable=False,name="batch_normalization2"+str(idx))
  ...
  def step(self, session, encoder_inputs, isTraining=True):
    ...
    return outputs[0]  # No gradient norm
